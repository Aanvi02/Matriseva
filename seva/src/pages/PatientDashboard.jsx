...
const getPatientByEmail= (email) => getPatients().find(p => p.patientEmail === email) || null;
const savePatient      = (p) => {
...
// ── 4-step self-registration form for patient ────────────────
const STEPS = [
...
function SelfRegistrationForm({ user, onDone }) {
  ...
  const handleNext = () => { if(!validate()) return; if(step<4) { setStep(step+1); window.scrollTo({top:0}); } else handleSubmit(); };
    ...
    <div style={{ minHeight:"100vh", background:C.cream, fontFamily:"'DM Sans','Segoe UI',sans-serif", padding:"28px 16px" }}>
      ...
      <div style={{ maxWidth:700, margin:"0 auto", background:C.white, borderRadius:18, boxShadow:"0 8px 36px rgba(0,0,0,0.08)", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"16px 32px", borderTop:"1px solid "+C.border, background:C.cream }}>
          ...
          <button onClick={handleNext} style={{ padding:"10px 26px", borderRadius:9, border:"none", background:C.saffron, color:"white", fontSize:14, fontWeight:700, cursor:"pointer", boxShadow:"0 4px 12px rgba(232,98,26,0.3)" }}>
            {step===4 ? "✓ Submit →" : "Next →"}
          </button>
        </div>
      </div>
...
export default function PatientDashboard() {
  ...
  useEffect(() => {
    setProfile(getPatientByEmail(user?.email));
    setLoading(false);
  }, []);
  ...
  // No profile → full 4-step self-registration form
  if (!profile) return <SelfRegistrationForm user={user} onDone={(p) => { setProfile(p); navigate("/portal"); }} />;
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
          ...
          <div style={{ background:C.white, borderRadius:14, padding:20, border:"1px solid "+C.border }}>
            ...
            {profile.visits?.length > 0 ? profile.visits.map((v,i) => (
              ...
              <div style={{ textAlign:"center", padding:"20px 0", color:C.muted, fontSize:13 }}>No visits recorded yet<br/><span style={{ fontSize:12 }}>Your ASHA worker will update after visits</span></div>
            )}
          </div>
        </div>
      </div>
